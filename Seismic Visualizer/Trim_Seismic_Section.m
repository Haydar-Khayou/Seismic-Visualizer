function Trim_Seismic_Section(handles)
...
%% Get Data
global Data_Trim
Data= Data_Trim;   % Get the initial Data

%% Get current Axis
Curr_Axis= handles.axes3;
if strcmp(handles.Distance_Axis.State, 'on')
    Curr_Axis.XLim= ((Curr_Axis.XLim/Data.Trace_Offset)*Data.StepLine)+min(Data.X_axis);
end
Sample_Interval= Data.step;

%% Prepare X-Axis Limits
Trace_Axis_First= min(Curr_Axis.XLim);
...
Trace_Axis_Last= max(Curr_Axis.XLim);
...
Time_Axis_First= min(Curr_Axis.YLim);
...
Y_Sample_First= ((Time_Axis_First - Data.first_Time)/Sample_Interval)+1;
Y_Sample_First= round(Y_Sample_First);

Time_Axis_Last= max(Curr_Axis.YLim);
...
Y_Sample_Last= ((Time_Axis_Last - Data.first_Time)/Sample_Interval)+1;
